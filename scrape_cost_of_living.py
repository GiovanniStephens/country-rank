...
def get_numbeo_countries():
    ...
    for i in range(len(countries)):
        try:
            std_countries[countries[i].text] = pycountry.countries.search_fuzzy(countries[i].text)[0].name
        except:
            pass
...
def get_country_cost_of_living(country, percentile):
    ...
    cleaned_table = clean_numbeo_table(table)
    if check_enough_data(cleaned_table) > 0.9:
        print(str(percentile)+ 'th percentile weekly cost of living in ' + country + ': ' + \
             str(round(get_cost_of_living(cleaned_table, percentile=percentile),2)))
    else:
